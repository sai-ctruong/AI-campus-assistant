...
const RECENT_DOCS = ["Introduction to Neural Networks.pdf", "Lecture_Notes_Week_3.pdf"];
...
export function AppSidebar({ active, onNavigate }: Props) {
  return (
    <aside className="flex h-full w-64 shrink-0 flex-col border-r border-outline-variant bg-surface px-4 py-5">
      ...
      {/* Recent docs (chỉ mang tính minh hoạ) */}
        ...
        <ul className="mt-3 flex flex-col gap-2">
          {RECENT_DOCS.map((d) => (
            <li key={d} className="flex items-center gap-2 text-sm text-on-surface-variant">
              <Icon name="description" size={16} className="shrink-0 text-outline" />
